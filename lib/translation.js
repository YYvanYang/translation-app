...
async function getCompletion(prompt, systemMessage = "You are a helpful assistant.", model = "gpt-4", temperature = 0.3, jsonMode = false) {
  const messages = [
...
function numTokensInString(inputStr, encodingName = "cl100k_base") {
  ...
  return encoding.encode(inputStr).length;
}
